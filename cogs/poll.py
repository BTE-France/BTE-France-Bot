...
emoji_list = [[f'{i+1}\N{VARIATION SELECTOR-16}\N{COMBINING ENCLOSING KEYCAP}', nums[i]] for i in range(len(nums))]
emoji_list += [['\N{REGIONAL INDICATOR SYMBOL LETTER A}', ':regional_indicator_a:'],
                ...
                if self.title is not None:
                    if len(self.choices) <= len(emoji_list):
                        if len(self.choices) != 0:
                            ...
                            embed.description = '\n'.join([f'{emojis[i][1]} {self.choices[i]}' for i in range(len(self.choices))])
