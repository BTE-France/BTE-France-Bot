...
class RandomCommands(interactions.Extension):
    ...
    async def ping(self, ctx: interactions.SlashContext):
        "Ping"
        await ctx.send(embeds=create_info_embed(f"Pong! {int(self.bot.latency)}ms."), ephemeral=True)
    ...
    @interactions.slash_option(name="number", description="Nombre de messages à supprimer", opt_type=interactions.OptionType.INTEGER)
    ...
    @interactions.context_menu(name="Supprimer messages après", context_type=interactions.CommandType.MESSAGE)
    @interactions.slash_default_member_permission(interactions.Permissions.MANAGE_MESSAGES)
    ...
    async def on_guild_ban_add(self, _):
        ...
        audit_logs = await self.guild.fetch_audit_log(action_type=interactions.AuditLogEventType.MEMBER_BAN_ADD, limit=1)
        audit_log_entry = audit_logs.entries[0]
        mod_user = next((user for user in audit_logs.users if user.id == audit_log_entry.user_id), None)

        if not mod_user.bot:
            banned_user = next((user for user in audit_logs.users if user.id == audit_log_entry.target_id), None)
