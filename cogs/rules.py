...
class Rules(commands.Cog):
    ...
    async def ip_embed(self, ctx):
        ...
        embed.add_field(name="<:minecraft_heart:922583879469117491>", value="BTE Hub : ```buildtheearth.net``` __Puis faites une fois en jeu :__```/bt FR```", inline=True)
        embed.add_field(name="<:minecraft_heart:922583879469117491>", value="BTE France :```bte.thesmyler.fr```", inline=True)
        embed.add_field(name="__IP BEDROCK__", value="(Windows 10, consoles...)", inline=False)
        embed.add_field(name="<:bedrock:922583660761333850>", value="```bedrock.buildtheearth.net```Port:```19132```", inline=True)
