...
def create_embed(
    ...
    if footer_text:
        embed.set_footer(
            text=footer_text,
            icon_url=footer_image if footer_image else variables.BTE_FRANCE_ICON,
        )
    for field in fields:
...
def create_error_embed(text: str) -> interactions.Embed:
    return create_embed(description=text, color=0xFF0000)


def create_info_embed(text: str) -> interactions.Embed:
    return create_embed(description=text, color=0x787774)
